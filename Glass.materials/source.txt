...
Release along with source text, a website, an interpreter, cover art, a file of "Walkthrough" called "solution.txt", a file of "Making of..." called "Overview.html".
...
Every turn when the old lady is active: 
	...
	begin;
		...
		now the last subject is the current subject;
		now the current subject is the segue;
	otherwise; 
		now the current subject is the last subject;
...
Understand "shit", "fuck", "damn", "sod" as "[obscenity]". [* We have more or less preserved Inform's default understanding of obscenity, though testers tried a range of other interesting words.]

Understand "swear", "curse", "mention obscenities" as swearing obscenely.

Understand "bother", "curses", "drat", "darn" as "[mild obscenity]".
...
Swearing obscenely is speech.

Swearing mildly is an action applying to nothing.
Swearing mildly is speech.
Saying yes is speech. Saying no is speech. Saying sorry is speech.
...
Wondering is a scene. Wondering begins when Prologue ends. When Wondering begins: now the conversation set is the Table of Wondering Remarks; now the target subject is ball; now the current action is "Curiously".
...
Fitting is a scene. Fitting begins when Wondering ends. When Fitting begins: now the conversation set is the Table of Fitting Remarks; now the target subject is Theo; now marriage suggests Theo; now the current action is "Nervously".
...
Checking Lucinda begins when Prince unties boots. Fitting ends when Checking Lucinda begins. When Checking Lucinda begins: 
		now the left hand status line is "Trying the slipper on Lucinda";
		now the last subject is the current subject; now the current subject is Lucinda; now the target subject is Lucinda; now the conversation set is the Table of Lucinda Checking Remarks.
...
Checking Theodora begins when Prince unties Theo boots. Fitting ends when Checking Theodora begins. When Checking Theodora begins:
		now the left hand status line is "Trying the slipper on Theodora";
		now the last subject is the current subject; now the current subject is Theo; now the target subject is Theo; now the conversation set is the Table of Theodora Checking Remarks.
...
Checking Cinderella begins when Cinderella gets mentioned. When Checking Cinderella begins:
	now the last subject is the current subject; now the current subject is Cinderella; now the target subject is magic; now the conversation set is the Table of Cinderella Checking Remarks;
